...
public class Lists {
    ...
    public static ListNode copyUpperCase(ListNode head) {
    	...
    	// Skapar ett nytt huvud fšr den nya listan
    	ListNode newHead = new ListNode();
    	// Skapar tvŒ pekare fšr som ska vandra šver resp. lista
    	ListNode newPointer = newHead;
    	ListNode oldPointer = head.next;
    	// Loopar šver den gamla listan
    	while(oldPointer != null) {
    		// Om element i gamla listan Šr stor bokstav:
    		if( oldPointer.element >= 'A' && oldPointer.element <= 'Z' ) {
    			// Skapar en ny listNode som fŒr vŠrdet av gamla element
    			ListNode p = new ListNode();
    			p.element = oldPointer.element;
    			// Gšr next fšr nya listans pekare till den nya ListNoden
    			newPointer.next = p;
    			newPointer = newPointer.next;
    		}
    		// GŒr vidare i gamla listan
